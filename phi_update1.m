%mean field
function [U1,C2,fieldPhi] = phi_update1(m,x)
...
%         P_2(n,q)=1/(1+exp(-softmax*(d(q)-0.2*Qk)))*1/(1+exp(-softmax*(0.2*Qk-qbar(n))));
        P_3(n,q)=1/(1+exp(-softmax*(d(q)-0.2*Qk)));
...
for n=1:Nt
    for q=1:Nq
        for l=1:Nh
            U1(n,q,l)=Ik(n)*p_k(n)*(P_1(q)*(Qk-d(q))+P_3(n,q)*Qk);
%             U2(n)=p_sharing*qbar(n)*(N2(n)/N1(n));
            C1(n,q,l)=omega_4*x(n,q,l)+omega_5*x(n,q,l)^2;
            C2(n,q,l)=eta*(Qk*x(n,q,l)/Hc+Ik(n)*(P_1(q)*(Qk-d(q))/rate(n)+...
